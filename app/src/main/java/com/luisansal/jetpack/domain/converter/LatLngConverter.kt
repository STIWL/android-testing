...
class OwnLatLngConverter {

    @TypeConverter
    fun convertToEntityProperty(databaseValue: String?): com.luisansal.jetpack.domain.model.maps.LatLng? {
        if (databaseValue == null) {
            return null
        }
        val latLng = databaseValue.split(";".toRegex()).dropLastWhile({ it.isEmpty() }).toTypedArray()
        return com.luisansal.jetpack.domain.model.maps.LatLng(Double.valueOf(latLng[0]), Double.valueOf(latLng[1]))
    }

    @TypeConverter
    fun convertToDatabaseValue(entityProperty: com.luisansal.jetpack.domain.model.maps.LatLng?): String? {
        return if (entityProperty == null) null else entityProperty.latitude.toString() + ";" + entityProperty.longitude
    }
}
